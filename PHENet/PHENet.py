...
def ota_alignment_loss(feat_v: torch.Tensor, feat_t: torch.Tensor, margin: float = 0.15):
    """
...
class VTCM(nn.Module):
    ...
    def __init__(self,
        ...
        self.experts = nn.ModuleList([
            E_Spatial(embed_dim),  # Fix #3 已在类内完成
            E_Noise(embed_dim),
            E_Freq(embed_dim),  # Fix #4 已在类内完成
        ])
    ...
    def forward(self, fuse_list, T, decoder_out, epoch=0, total_epochs=100):
        ...
        for i, (fuse, vis_proj) in enumerate(zip(fuse_list, self.vis_projs)):
            ...
            if self.training:
                ot_intra, hinge = ota_alignment_loss(feat_global, T, self.margin)
...
class VCM(nn.Module):
    ...
    def __init__(self, dim: int, n_experts: int = 4, top_k: int = 2,
                 d_state: int = 16):
        super().__init__()
        assert n_experts == 4, "VCM 固定使用四位异质专家"
        self.N   = n_experts
        self.K   = top_k
        self.eps = 1e-6
    ...
    def forward(self, Fm1: torch.Tensor, Fm2: torch.Tensor):
        ...
        if self.training:
            ...
            # 小系数加权原始损失，避免 VCM 主导总损失
            # L_wb 自然量级 ~0.1-0.5，直接用；L_div / L_cons 乘小系数压制
            loss_vcm = (0.1 * L_wb
                         + 0.05 * lam * L_div
                         + 0.05 * (1 - lam) * L_cons)
            if torch.isnan(loss_vcm):
                loss_vcm = torch.tensor(0.0, device=Fm1.device)
        else:
            loss_vcm = torch.tensor(0.0, device=Fm1.device)
...
class TPM(nn.Module):
    def __init__(self, dim=512, num_heads=8, num_layers=6,
        ...
        self.layers = nn.ModuleList([
            TPMBlock(dim, num_heads, num_experts, dropout)
...
class PLGS(nn.Module):
    """TextBranch + TPM + TextClassEmbedder (Bug2 修复)"""
...
class PHENet(nn.Module):
    ...
    def __init__(self,
        ...
        self.vcm1 = VCM(dim=144, n_experts=4, top_k=2)
        self.vcm2 = VCM(dim=288, n_experts=4, top_k=2)
        self.vcm3 = VCM(dim=576, n_experts=4, top_k=2)
        self.vcm4 = VCM(dim=1152, n_experts=4, top_k=2)
    ...
    def forward(self, x: torch.Tensor, epoch=0, total_epochs=100):
        ...
        for _m in [self.vcm1, self.vcm2, self.vcm3, self.vcm4]:
            _m.set_epoch(epoch, total_epochs)
        fuse1, l1 = self.vcm1(f1_1, f2_1)
        fuse2, l2 = self.vcm2(f1_2, f2_2)
        fuse3, l3 = self.vcm3(f1_3, f2_3)
        fuse4, l4 = self.vcm4(f1_4, f2_4)
        loss_vcm = l1 + l2 + l3 + l4
        ...
        if self.use_text_align:
            ...
            if self.training:
                ...
                if epoch != self.last_printed_epoch:
                    avg_vcm = self.total_vcm / self.batch_count
                    ...
                    print(f"[Epoch {epoch:2d}] vcm: {avg_vcm:.4f} | load: {avg_load:.4f} | ot: {avg_ot:.4f} | hinge: {avg_hinge:.4f}")
                ...
                return out, loss_vcm, load_loss, ot_intra_loss, hinge_loss
            else:
                out = self.text_align(
                    fuse_list=[fuse1, fuse2, fuse3, fuse4],
                    T=T,
...
if __name__ == '__main__':
    ...
    model.train()
    out, loss_vcm, load_loss, ot_loss, hinge_loss = model(test_input, )
    print("========= 修复版全流程前向验证 =========")
    print(f"✅ 分割输出尺寸:                   {out.shape}")
    print(f"✅ 基础融合损失 loss_vcm:         {loss_vcm.item():.4f}")
